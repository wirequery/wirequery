...
import com.wirequery.spring6.TraceCache
...
class WireQueryConfig {
    ...
    @Bean
    @ConditionalOnMissingBean
    fun queryCompiler(config: WireQueryConfigurationProperties) =
        QueryCompiler(
            ExpressionCompiler(),
            QueryAuthorizer(
    ...
    private fun mapResourceAuthorizationSettings(
        resourceAuthorizationSettings: List<WireQueryConfigurationProperties.ResourceAuthorizationSetting>,
    ): List<QueryAuthorizer.ResourceAuthorizationSetting> {
    ...
    @Bean
    @ConditionalOnMissingBean
    fun simpleObjectMasker(config: WireQueryConfigurationProperties): ObjectMasker =
        SimpleObjectMasker(
            defaultObjectMapper,
            ClassAnalyzingMaskDeterminer(
                unmaskByDefault = config.maskSettings.unmaskByDefault,
                additionalClasses =
                    config.maskSettings.classes.associate { clazz ->
                        clazz.name to
                            ClassAnalyzingMaskDeterminer.AdditionalClass(
                                mask = clazz.mask,
                                unmask = clazz.unmask,
                                fields =
                                    clazz.fields.associate { field ->
                                        field.name to
                                            ClassAnalyzingMaskDeterminer.AdditionalField(
                                                mask = field.mask,
                                                unmask = field.unmask,
                                            )
                                    },
    ...
    fun wireQueryAdapter(
        ...
        config.connection?.let { conn ->
            WireQueryAdapter(
                wireQueryStub =
                    WirequeryServiceGrpc.newStub(
                ...
                connectionSettings =
                    WireQueryAdapter.ConnectionSettings(
